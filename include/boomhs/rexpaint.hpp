...
struct RexError
{
  ...
  std::string to_string() const { return fmt::format("{code: '%i', msg: '%s'}", code, message); }
};
...
public:
  ...
  static Result<RexImage, RexError> load(std::string const&);
  ...
  RexTile& get_tile(LayerNumber const layer, int const x,
                    int const y); // { return &layers.at(layer).tiles.at(y + (x * height)); };
  ...
  RexTile& get_tile(LayerNumber const layer,
                    int const         index); // { return &layers.at(layer).tiles.at(index); };
