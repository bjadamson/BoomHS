...
Edges
calculate_edges(TilePosition const& tpos, uint64_t const, uint64_t const, uint64_t const, uint64_t);
...
floodfill(TileGrid& tgrid, TileType const type)
{
  ...
  FOR(x, w)
  {
    FOR(y, h) { tgrid.data(x, y).type = type; }
  }
}
...
visit_each(TileGrid const& tgrid, FN const& fn, Args&&... args)
{
  ...
  FOR(x, w)
  {
    FOR(y, h) { fn(TilePosition{x, y}, std::forward<Args>(args)...); }
  }
}
...
template <typename FN>
void
visit_neighbors(TileGrid const& tgrid, TilePosition const& pos, FN const& fn,
                TileLookupBehavior const behavior)
{
...
public:
  ...
  static MapEdge random_edge(stlw::float_generator&);
};
