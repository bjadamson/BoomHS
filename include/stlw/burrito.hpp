...
struct burrito
{
  ...
  template <typename T>
  explicit constexpr burrito(T&& t)
      : value(std::move(t))
  {
  }
...
template <template <class, size_t> typename C, typename T, size_t N>
auto constexpr make_burrito(C<T, N>&& arr)
{
...
template <typename C>
auto constexpr make_burrito(C&& c)
{
  return burrito<std::decay_t<C>, container_tag>{std::move(c)};
}

template <typename... T>
auto constexpr make_burrito(std::tuple<T...>&& t)
{
  using U = std::tuple<T...>;
...
template <typename... T>
auto constexpr make_burrito(T&&... t)
{
  using U = std::tuple<T...>;
...
auto
map_impl(C<T, std::allocator<T>> const& c, FN const& fn)
{
...
template <typename U, typename FN>
auto
map(burrito<U, container_tag> const& b, FN const& fn)
{
