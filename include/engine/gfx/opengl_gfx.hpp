...
{
  char const* filename;
  vertex_shader_filename(char const* f) : filename(f) {}
};
struct fragment_shader_filename
{
  char const* filename;
  fragment_shader_filename(char const* f) : filename(f) {}
};
...
class renderer
{
  ...
  renderer(W &&w) : window_(std::move(w)),
  VAO(0),
  VBO(0),
...
public:
  ...
  void
  render(program_handle const& program_handle, GLuint const VAO)
  {
  ...
  void
  draw(GLfloat const the_vertices[12])
  {
    //print_matrix(vertices);
    auto const send_vertices_gpu = [](auto const& vbo, auto const vinfo)//GLfloat const vertices[12])
    {
    ...
    struct vertices_info
    {
      ...
      vertices_info(int const s, GLfloat const* b) : size_in_bytes(s), buffer(b) {}
    };
