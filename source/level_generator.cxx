    ...
    stlw::float_generator &rng, TileType const type)
{
  ...
  while(!room && (trials < max_tries)) {
    room = try_create_room(rgconfig, tmap, rng, type);
    ++trials;
  }
  return room;
}
...
void
create_h_tunnel(int const x1, int const x2, int const y, TileType const type, TileMap &tmap)
{
...
void
create_v_tunnel(int const y1, int const y2, int const x, TileType const type, TileMap &tmap)
{
...
create_rooms(int const width, int const height, TileMap &tmap, stlw::float_generator &rng)
{
  ...
  RoomGenConfig const rgconfig{width, height, rects};
  {
    ...
    FOR(i, 10) {
      auto const prev_center = rects.front().center();

      // 2. Create second river room
      MAKEOPT(auto const new_room, create_room(MAX_NUM_CREATE_TRIES, rgconfig, tmap, rng,
            TileType::RIVER));
      auto const new_center = new_room.center();

      // Connect them with river passage ways
      create_h_tunnel(prev_center.x, new_center.x, prev_center.y, TileType::RIVER, tmap);
      create_v_tunnel(prev_center.y, new_center.y, new_center.x, TileType::RIVER, tmap);
    }
  }
  ...
  FOR(_, MAX_ROOMS) {
    ...
    if (rng.gen_bool()) {
      ...
      create_h_tunnel(prev_center.x, new_center.x, prev_center.y, TileType::FLOOR, tmap);
      create_v_tunnel(prev_center.y, new_center.y, new_center.x, TileType::FLOOR, tmap);
    } else {
      // first move vertically, then horizontally
      create_v_tunnel(prev_center.y, new_center.y, prev_center.x, TileType::FLOOR, tmap);
      create_h_tunnel(prev_center.x, new_center.x, new_center.y, TileType::FLOOR, tmap);
    }
    add_room(new_room);
  }
