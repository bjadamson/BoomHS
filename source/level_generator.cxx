...
#define CREATE_ROOM_OR_YIELD(var, tries, roomgenconfig, tilemap, random) \
  EVAL_INTO_VAR_OR(var, , [](auto const&) { return boost::none; })
...
create_rooms(int const width, int const height, TileMap &tmap, stlw::float_generator &rng)
{
  ...
  DO_TRY_OR_ELSE_RETURN_DEFAULT_T(auto const new_room,
      create_room(MAX_NUM_CREATE_TRIES, rgconfig, tmap, rng),
      boost::none);
