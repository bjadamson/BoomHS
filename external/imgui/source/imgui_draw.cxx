...
#pragma clang diagnostic ignored "-Wdouble-promotion" // warning: implicit conversion from 'float'
                                                      // to 'double' when passing argument to
                                                      // function
#endif
...
void
ImDrawList::AddCallback(ImDrawCallback callback, void *callback_data)
{
...
// etc. to not have to perform any check so we always have a command ready in the stack.
// The cost of figuring out if a new command has to be added or if we can merge is paid in those
...
ImDrawList::UpdateClipRect()
{
  ...
  if (!curr_cmd || (curr_cmd->ElemCount != 0 &&
...
// widget culling)
void
ImDrawList::PushClipRect(ImVec2 cr_min, ImVec2 cr_max, bool intersect_with_current_clip_rect)
{
...
void
ImDrawList::PushTextureID(const ImTextureID &texture_id)
{
...
void
ImDrawList::ChannelsSplit(int channels_count)
{
  ...
  // debugger but we don't strictly need to.
  // When we switch to the next channel, we'll copy _CmdBuffer/_IdxBuffer into _Channels[0] and then
  // _Channels[1] into _CmdBuffer/_IdxBuffer
...
ImDrawList::ChannelsMerge()
{
  ...
  UpdateClipRect(); // We call this instead of AddDrawCmd(), so that empty channels won't produce an
                    // extra draw call.
  _ChannelsCount = 1;
}

void
ImDrawList::ChannelsSetCurrent(int idx)
{
...
// not underflow)
void
ImDrawList::PrimReserve(int idx_count, int vtx_count)
{
...
void
ImDrawList::PrimRect(const ImVec2 &a, const ImVec2 &c, ImU32 col)
{
...
void
ImDrawList::PrimRectUV(const ImVec2 &a, const ImVec2 &c, const ImVec2 &uv_a, const ImVec2 &uv_c,
                       ImU32 col)
{
...
void
ImDrawList::PrimQuadUV(const ImVec2 &a, const ImVec2 &b, const ImVec2 &c, const ImVec2 &d,
...
void
ImDrawList::AddPolyline(const ImVec2 *points, const int points_count, ImU32 col, bool closed,
...
void
ImDrawList::AddConvexPolyFilled(const ImVec2 *points, const int points_count, ImU32 col,
                                bool anti_aliased)
{
  const ImVec2 uv = GImGui->FontTexUvWhitePixel;
...
void
ImDrawList::PathArcToFast(const ImVec2 &centre, float radius, int a_min_of_12, int a_max_of_12)
{
...
void
ImDrawList::PathArcTo(const ImVec2 &centre, float radius, float a_min, float a_max,
                      int num_segments)
{
  if (radius == 0.0f) {
    _Path.push_back(centre);
...
static void
PathBezierToCasteljau(ImVector<ImVec2> *path, float x1, float y1, float x2, float y2, float x3,
...
void
ImDrawList::PathBezierCurveTo(const ImVec2 &p2, const ImVec2 &p3, const ImVec2 &p4,
                              int num_segments)
{
...
void
ImDrawList::PathRect(const ImVec2 &a, const ImVec2 &b, float rounding, int rounding_corners)
{
  rounding =
      ImMin(rounding,
            fabsf(b.x - a.x) *
...
void
ImDrawList::AddLine(const ImVec2 &a, const ImVec2 &b, ImU32 col, float thickness)
{
...
void
ImDrawList::AddRect(const ImVec2 &a, const ImVec2 &b, ImU32 col, float rounding,
...
void
ImDrawList::AddRectFilled(const ImVec2 &a, const ImVec2 &b, ImU32 col, float rounding,
...
void
ImDrawList::AddRectFilledMultiColor(const ImVec2 &a, const ImVec2 &c, ImU32 col_upr_left,
...
void
ImDrawList::AddQuad(const ImVec2 &a, const ImVec2 &b, const ImVec2 &c, const ImVec2 &d, ImU32 col,
                    float thickness)
{
  if ((col & IM_COL32_A_MASK) == 0)
    return;
...
void
ImDrawList::AddQuadFilled(const ImVec2 &a, const ImVec2 &b, const ImVec2 &c, const ImVec2 &d,
                          ImU32 col)
{
  if ((col & IM_COL32_A_MASK) == 0)
    return;
...
void
ImDrawList::AddTriangle(const ImVec2 &a, const ImVec2 &b, const ImVec2 &c, ImU32 col,
                        float thickness)
{
  if ((col & IM_COL32_A_MASK) == 0)
    return;
...
void
ImDrawList::AddTriangleFilled(const ImVec2 &a, const ImVec2 &b, const ImVec2 &c, ImU32 col)
{
  if ((col & IM_COL32_A_MASK) == 0)
    return;
...
void
ImDrawList::AddCircle(const ImVec2 &centre, float radius, ImU32 col, int num_segments,
                      float thickness)
{
  if ((col & IM_COL32_A_MASK) == 0)
    return;
...
void
ImDrawList::AddCircleFilled(const ImVec2 &centre, float radius, ImU32 col, int num_segments)
{
  if ((col & IM_COL32_A_MASK) == 0)
    return;
...
void
ImDrawList::AddBezierCurve(const ImVec2 &pos0, const ImVec2 &cp0, const ImVec2 &cp1,
...
void
ImDrawList::AddText(const ImFont *font, float font_size, const ImVec2 &pos, ImU32 col,
                    ...
                    const ImVec4 *cpu_fine_clip_rect)
{
  ...
  // IMPORTANT: This is one of the few instance of breaking the encapsulation of ImDrawList, as we
  // pull this from ImGui state, but it is just SO useful.
...
void
ImDrawList::AddText(const ImVec2 &pos, ImU32 col, const char *text_begin, const char *text_end)
{
  AddText(NULL, 0.0f, pos, col, text_begin, text_end);
}

void
ImDrawList::AddImage(ImTextureID user_texture_id, const ImVec2 &a, const ImVec2 &b,
...
void
ImDrawList::AddImageQuad(ImTextureID user_texture_id, const ImVec2 &a, const ImVec2 &b,
...
void
ImDrawList::AddImageRounded(ImTextureID user_texture_id, const ImVec2 &a, const ImVec2 &b,
                            ...
                            int rounding_corners)
{
  ...
  ImGui::ShadeVertsLinearUV(VtxBuffer.Data + vert_start_idx, VtxBuffer.Data + vert_end_idx, a, b,
                            uv_a, uv_b, true);
...
void
ImDrawData::ScaleClipRects(const ImVec2 &scale)
{
  for (int i = 0; i < CmdListsCount; i++) {
    ImDrawList *cmd_list = CmdLists[i];
...
void
ImGui::ShadeVertsLinearColorGradientKeepAlpha(ImDrawVert *vert_start, ImDrawVert *vert_end,
                                              ...
                                              ImU32 col1)
{
  ...
  for (ImDrawVert *vert = vert_start; vert < vert_end; vert++) {
    ...
    int r =
        ImLerp((int)(col0 >> IM_COL32_R_SHIFT) & 0xFF, (int)(col1 >> IM_COL32_R_SHIFT) & 0xFF, t);
    int g =
        ImLerp((int)(col0 >> IM_COL32_G_SHIFT) & 0xFF, (int)(col1 >> IM_COL32_G_SHIFT) & 0xFF, t);
    int b =
        ImLerp((int)(col0 >> IM_COL32_B_SHIFT) & 0xFF, (int)(col1 >> IM_COL32_B_SHIFT) & 0xFF, t);
...
// vert_start..vert_end going left to right) so we can break as soon as we are out the gradient
// bounds.
void
ImGui::ShadeVertsLinearAlphaGradientForLeftToRightText(ImDrawVert *vert_start, ImDrawVert *vert_end,
...
void
ImGui::ShadeVertsLinearUV(ImDrawVert *vert_start, ImDrawVert *vert_end, const ImVec2 &a,
                          const ImVec2 &b, const ImVec2 &uv_a, const ImVec2 &uv_b, bool clamp)
{
  const ImVec2 size = b - a;
  const ImVec2 uv_size = uv_b - uv_a;
  const ImVec2 scale =
      ImVec2(size.x ? (uv_size.x / size.x) : 0.0f, size.y ? (uv_size.y / size.y) : 0.0f);
...
void
ImFontAtlas::GetTexDataAsAlpha8(unsigned char **out_pixels, int *out_width, int *out_height,
...
void
ImFontAtlas::GetTexDataAsRGBA32(unsigned char **out_pixels, int *out_width, int *out_height,
                                int *out_bytes_per_pixel)
{
...
ImFont *
ImFontAtlas::AddFont(const ImFontConfig *font_cfg)
{
  ...
  if (!font_cfg->MergeMode) {
    ...
    IM_ASSERT(!Fonts.empty()); // When using MergeMode make sure that a font has already been added
...
static void
Decode85(const unsigned char *src, unsigned char *dst)
{
  while (*src) {
    unsigned int tmp =
        Decode85Byte(src[0]) +
...
ImFont *
ImFontAtlas::AddFontDefault(const ImFontConfig *font_cfg_template)
{
  ...
  ImFont *font = AddFontFromMemoryCompressedBase85TTF(ttf_compressed_base85, font_cfg.SizePixels,
                                                      &font_cfg, GetGlyphRangesDefault());
  return font;
}

ImFont *
ImFontAtlas::AddFontFromFileTTF(const char *filename, float size_pixels,
...
ImFont *
ImFontAtlas::AddFontFromMemoryTTF(void *ttf_data, int ttf_size, float size_pixels,
...
int
ImFontAtlas::AddCustomRectRegular(unsigned int id, int width, int height)
{
...
int
ImFontAtlas::AddCustomRectFontGlyph(ImFont *font, ImWchar id, int width, int height,
...
void
ImFontAtlas::CalcCustomRectUV(const CustomRect *rect, ImVec2 *out_uv_min, ImVec2 *out_uv_max)
{
...
void
ImFontAtlasBuildMultiplyCalcLookupTable(unsigned char out_table[256], float in_brighten_factor)
{
...
void
ImFontAtlasBuildMultiplyRectAlpha8(const unsigned char table[256], unsigned char *pixels, int x,
...
bool
ImFontAtlasBuildWithStbTruetype(ImFontAtlas *atlas)
{
  ...
  // We need a width for the skyline algorithm. Using a dumb heuristic here to decide of width. User
  // can override TexDesiredWidth and TexGlyphPadding if they wish.
  // Width doesn't really matter much, but some API/GPU have texture size limitations and increasing
  ...
  stbtt_pack_range *buf_ranges =
      (stbtt_pack_range *)ImGui::MemAlloc(total_ranges_count * sizeof(stbtt_pack_range));
  ...
  // First font pass: pack all glyphs (no rendering at this point, we are working with rectangles in
  // an infinitely tall texture at this point)
  for (int input_i = 0; input_i < atlas->ConfigData.Size; input_i++) {
    ImFontConfig &cfg = atlas->ConfigData[input_i];
    ImFontTempBuildData &tmp = tmp_array[input_i];
  ...
  for (int input_i = 0; input_i < atlas->ConfigData.Size; input_i++) {
    ...
    for (int i = 0; i < tmp.RangesCount; i++) {
      ...
      for (int char_idx = 0; char_idx < range.num_chars; char_idx += 1) {
        ...
        stbtt_GetPackedQuad(range.chardata_for_range, atlas->TexWidth, atlas->TexHeight, char_idx,
                            &dummy_x, &dummy_y, &q, 0);
        dst_font->AddGlyph((ImWchar)codepoint, q.x0 + off_x, q.y0 + off_y, q.x1 + off_x,
                           q.y1 + off_y, q.s0, q.t0, q.s1, q.t1, pc.xadvance);
      }
    }
  }
...
void
ImFontAtlasBuildRegisterDefaultCustomRects(ImFontAtlas *atlas)
{
...
void
ImFontAtlasBuildSetupFont(ImFontAtlas *atlas, ImFont *font, ImFontConfig *font_config, float ascent,
                          float descent)
{
...
void
ImFontAtlasBuildPackCustomRects(ImFontAtlas *atlas, void *pack_context_opaque)
{
...
static void
ImFontAtlasBuildRenderDefaultTexData(ImFontAtlas *atlas)
{
...
void
ImFontAtlasBuildFinish(ImFontAtlas *atlas)
{
...
const ImWchar *
ImFontAtlas::GetGlyphRangesChinese()
{
  static const ImWchar ranges[] = {
      0x0020, 0x00FF, // Basic Latin + Latin Supplement
...
ImFontAtlas::GetGlyphRangesJapanese()
{
  ...
  // 0x4E00. Each offset has an implicit +1.
  ...
  // Note that you may use ImFontAtlas::GlyphRangesBuilder to create your own ranges, by merging
  // existing ranges or adding new characters.
...
void
ImFontAtlas::GlyphRangesBuilder::AddText(const char *text, const char *text_end)
{
...
void
ImFontAtlas::GlyphRangesBuilder::BuildRanges(ImVector<ImWchar> *out_ranges)
{
...
void
ImFont::SetFallbackChar(ImWchar c)
{
  FallbackChar = c;
  BuildLookupTable();
}
...
void
ImFont::AddGlyph(ImWchar codepoint, float x0, float y0, float x1, float y1, float u0, float v0,
...
void
ImFont::AddRemapChar(ImWchar dst, ImWchar src, bool overwrite_dst)
{
...
const ImFontGlyph *
ImFont::FindGlyph(ImWchar c) const
{
...
const char *
ImFont::CalcWordWrapPositionA(float scale, const char *text, const char *text_end,
...
ImVec2
ImFont::CalcTextSizeA(float size, float max_width, float wrap_width, const char *text_begin,
                      const char *text_end, const char **remaining) const
{
  ...
  while (s < text_end) {
    ...
    const float char_width =
        ((int)c < IndexAdvanceX.Size ? IndexAdvanceX[(int)c] : FallbackAdvanceX) * scale;
...
void
ImFont::RenderChar(ImDrawList *draw_list, float size, ImVec2 pos, ImU32 col, unsigned short c) const
{
  if (c == ' ' || c == '\t' || c == '\n' ||
...
void
ImFont::RenderText(ImDrawList *draw_list, float size, ImVec2 pos, ImU32 col,
...
void
ImGui::RenderRectFilledRangeH(ImDrawList *draw_list, const ImRect &rect, ImU32 col,
